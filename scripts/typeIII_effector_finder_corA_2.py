...
def getGeneLocationCRISPRCas(cctyper_gene_positions, contig, position_order):
    ...
    positions = cctyper_gene_positions
...
#Go through all genes in the cctyper output and look for low e-values
for key, value in type_III_loci.items():
    ...
    for id, gene in enumerate(genelist):
        ...
        if ("Cas10" in gene) & (float(evaluelist[id]) <= Cas10_evalue_threshold): #get information on Cas10 cyclase motif, if we trust that this is Cas10
            ...
            cas10_start = cas10_global_start_end_coordinates[0]
            cas10_end = cas10_global_start_end_coordinates[1]
        ...
        if gene_info["evalue"] > cctyper_evalue_threshold:
            ...
            global_start_end_coordinates = getGeneLocationCRISPRCas(cctyper_gene_positions, value["contig"], gene_pos_list[id])
            gff_protein_object = gff.region(seqid=value["contig"], start = global_start_end_coordinates[0], end = global_start_end_coordinates[1], completely_within = True, featuretype = "CDS") #find corresponding coordinates from gff file
