...
parser.add_argument('-rn', '--ring_nuclease', help='ring nuclease True of False', required=False)
...
for line in hmm_target_file:
    ...
    if ("#" not in effector):
        if ring_nuclease == "True":
            effector = re.split("_", effector)[0] #ring nucleae currently lack the cOA type in their name (29.11.2020)
        else:
...
for index, row in hmm.iterrows(): #check each row in hmm result
    ...
    if (protein_length > min_lengths[precise_hit] and protein_length < max_lengths[precise_hit]):
        print("...Protein " + protein_id + " is long/short enough at " + str(protein_length) + " AA")
        #
        ...
        precise_hit_and_score = {"RN": precise_hit,
                                "clade": clade,
                                "score": target_score, 
                                "hit_type": hit_type,
                                "sample": sample,
        ...
        if protein_id in protein_to_effector:
            ...
            print("Checking the score of the existing hit (" + str(protein_to_effector[protein_id][0]["RN"] + ") and comparing to new one: " + str(protein_to_effector[protein_id][0]["score"]) + " vs new score of " + str(target_score) + " in " + str(precise_hit)))
...
for key, value in protein_to_effector.items(): #iterate through all proteins
    if len(value) > 0: #if the protein has a hit in the hmm
        protein_id = key
        precise_hit = value[0]["RN"] #get the effector from the protein_to_effector dictionary
        ...
        plottable_effector_table = plottable_effector_table.append({"protein_id": protein_id, "effector": precise_hit, "sample": sample, "type": hit_type, "sequence": protein_sequence}, ignore_index=True)
...
print("Saving effector_to_protein_df to " + str(os.path.join(output_folder, sample + "_effector_to_protein.tsv")))
effector_to_protein_df.to_csv(os.path.join(output_folder, sample + "_effector_to_protein.tsv"), index=True, sep = '\t', header = False)
protein_to_effector_df.to_csv(os.path.join(output_folder, sample + "_protein_to_effector.tsv"), index=True, sep = '\t', header = False)
