...
if mode == "pre_hmm":
    ...
    print(
        "Saving protein multifasta file to "
        + str(os.path.join(output_folder, locus + "_contig_proteins.faa"))
    )
    SeqIO.write(
        protein_seqs,
        os.path.join(output_folder, locus + "_contig_proteins.faa"),
        "fasta",
    )
...
if mode == "post_hmm":
    ...
    for index, row in hmm.iterrows():  # check each row in hmm result
        ...
        if (
            ...
            if "#" in row["target_name"]:
                ...
                if (
                    ...
                    print(
                        "...Ring nuclease detected with hashtag: "
                        + str(row["target_name"])
                    )
                ...
                if precise_hit == "cam1":
                    ...
                    if has_TM == False:
                        print(
                            "...cam1-annotated protein does not have a transmembrane domain. Skipping"
                        )
                        continue
                    else:
                        print(
                            "Protein has this many transmembrane segments: "
                            + str(tmhmm_count)
                        )
                if precise_hit == "csx1":
                    ...
                    if has_TM == True:
                        print(
                            "Csx1 annotated protein has a transmembrane domain. Skipping"
                        )
                        continue
                if precise_hit == "tirsaved":
                    ...
                    if float(row["score_fullseq"]) < float(500):
                        ...
                        print(
                            "...tirsaved detected with bitscore above 500. Adding to discovered effectors"
                        )
            elif "blender" in row["target_name"]:
                # this is a special temp case
                hit_type = "rng"
                precise_hit = "blender"
        ...
        print(
            "...Discovered cA type in hmm: " + str(hit_type) + " (" + precise_hit + ")"
        )

        proteinInfo = getProteinCoordinates(
            protein_id, db
        )  # get the start and stop coordinates of the protein
        ...
        if (
            start >= cas_operon_start
            and stop <= cas_operon_end
            and protein_length > min_lengths[precise_hit]
            and protein_length < max_lengths[precise_hit]
        ):
            print(
                "...Protein "
                + protein_id
                + " is in the cas operon and is long/short enough at "
                + str(protein_length)
                + " AA"
            )
            #
            ...
            if protein_id in protein_to_effector:
                ...
                print(
                    ...
                    + str(
                        protein_to_effector[protein_id][0]["effector"]
    ...
    print(
        ...
        + str(os.path.join(output_folder, locus + "_effector_to_protein.tsv"))
    )
    effector_to_protein_df.to_csv(
        os.path.join(output_folder, locus + "_effector_to_protein.tsv"),
        index=True,
        sep="\t",
        header=False,
    )
    protein_to_effector_df.to_csv(
        os.path.join(output_folder, locus + "_protein_to_effector.tsv"),
        index=True,
        sep="\t",
        header=False,
    )
    ...
    plottable_effector_table.to_csv(
        effector_plot_data, index=False, sep="\t", header=True
    )
