...
'''
Searches type III loci for Csx19 as labeled by cctyper. These genes represent potential cA3 ring nucleases.
'''
...
for index, row in cctyper_df.iterrows():
    ...
    for i, gene in enumerate(row["Genes"]):
        ...
        if ("Csx19" in gene):
            print("Found Csx19 with E-value: " + gene + " with E-value: " + str(evalue))
            #get gene location
            ...
            csx19_object = protein_class(row["locus_id"], row["sample"], seq, length, row["E-values"][i], row["Positions"][i], row["locus_id"] + "_" + str(row["Positions"][i]), gene)
            csx19_proteins.append(csx19_object)

#write the unknown_proteins to a fasta file
with open(unknown_proteins_output, "w") as f:
    for protein in csx19_proteins:
        f.write(">" + protein.id + "\n")
...
unknown_proteins_df = pd.DataFrame([protein.to_dict() for protein in csx19_proteins])
